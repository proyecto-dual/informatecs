...
export default function AdminForgotPage() {
  ...
  return (
    <form onSubmit={handleSubmit} className="login-form">
      ...
      {status === "error" && (
        <p style={{ color: "#c0392b", fontSize: "0.82rem", marginTop: "0.25rem" }}>⚠️ {message}</p>
      )}
      {status === "success" && (
        <div style={{ background: "#eafaf1", border: "1px solid #27ae60", borderRadius: "8px", padding: "0.75rem 1rem", marginTop: "0.5rem" }}>
          <p style={{ color: "#27ae60", fontSize: "0.85rem", margin: 0 }}>✅ {message}</p>
        </div>
      )}
      {status !== "success" && (
      ...
      <button type="button" onClick={() => router.push("/login")}
        style={{ background: "none", border: "none", color: "#1b396a", fontSize: "0.85rem", cursor: "pointer", textDecoration: "underline", marginTop: "0.75rem", display: "block", width: "100%", textAlign: "center", padding: 0 }}>
