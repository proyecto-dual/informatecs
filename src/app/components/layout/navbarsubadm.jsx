...
export default function SubAdminSidebar({ open, setOpen }) {
  ...
  useEffect(() => {
    const cargarPermisos = async () => {
      ...
      try {
        ...
        if (data.permisos) setPermisosActivos(data.permisos);
      } catch {}
    };
    ...
    window.addEventListener("focus", cargarPermisos);

    return () => {
      clearInterval(intervalo);
      window.removeEventListener("focus", cargarPermisos);
    };
  }, []);
  ...
  const itemsVisibles = menuItems.filter((item) => tienePermiso(item.label));
